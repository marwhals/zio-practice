...
import utils.*
import zio.{Exit, Fiber, UIO, ZIO, ZIOAppDefault}
...
object Fibers extends ZIOAppDefault {
  ...
  // Generalise for different error types
  ...
  // spawn n fibers (= nFiles each fiber reads one file)
  // count the number of words in each file, then aggregate all the results together in one big number
  ...
  def wordCountParallel(n: Int): UIO[Int] = {
    val effects: Seq[ZIO[Any, Nothing, Int]] = (1 to n)
      .map(i => s"src/main/scala/resources/testfile_$i.txt")
...
//  def run = ZIO.succeed((1 to 10).foreach(i => generateRandomFile(s"src/main/scala/resources/testfile_${i}.txt")))
