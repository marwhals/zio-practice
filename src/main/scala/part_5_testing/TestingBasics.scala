...
object MyTestSpec extends ZIOSpecDefault {
  ...
  def spec = test("First Test") {
    val person = Person("Person", 99)

    // an assertion
    assert(person.spellName)(Assertion.equalTo("Personz"))
    // same
    assertTrue(person.spellName == "Person")
...
object MyFirstEffectTestSpec extends ZIOSpecDefault {
  def spec = test("First Effect Test") {
    val person = Person("Person", 101)
    assertZIO(person.saySomething)(Assertion.equalTo("Hi, I am a person"))
    assertZIO(person.saySomething)(Assertion.assertion("should be a greeting")(gr => gr == "Hi, I'm a person"))
    ...
    - Specialised
      ...
      - isEmptyString/ nonEmptyString, startWithString, matchesRegex for Strings
     */
  }
}
