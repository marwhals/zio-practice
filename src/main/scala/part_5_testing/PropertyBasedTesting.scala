 ...
 * - Validate statements with varying degrees of generality
 * Generators
 ...
 * - from ZIOs
 * - unfold
 * Term: Shrinking ---- reduce concrete example to most simple one which makes it fail the property
 */

object PropertyBasedTesting extends ZIOSpecDefault {

  // property based testing is used for "proofs" - see math theorems
  // shrinking ---- reduce concrete example to most simple one which makes it fail the property
  ...
  } yield Uuid(s"$part1-$part2-$part3-$part4")

  // general
  val randomGenerator = Gen.fromRandom(random => random.nextUUID)
  val effectGenerator = Gen.fromZIO(ZIO.succeed(42))
  val generalGenerator = Gen
    .unfoldGen(0)(i => Gen.const(i + 1).zip(Gen.stringN(i)(Gen.alphaNumericChar)))
