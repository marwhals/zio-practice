 ...
 * - Dependencies and layers allow you to pass a "mocked" dependency to ZIOs
 */
...
object SimpleDependencySpec extends ZIOSpecDefault {
  ...
  def spec = suite("A user survey application should...")(
    test("normalize user names") {
      ...
      assertZIO(surveyPreliminaryLogic)(Assertion.equalTo("bob"))
    }
  ).provide(ZLayer.fromZIO(mockedDatabase)) //Provide a mocked service to all tests requiring it
}
...
object BuiltInTestServicesSpec extends ZIOSpecDefault {
  def spec = suite("Checking built-in test services")(
    test("ZIO console application") { // TestConsole - "type" lines in and fetch all the output
      val logicUnderTest: Task[Vector[String]] = for {
        _ <- TestConsole.feedLines("Bob")
        _ <- DummyConsoleApplication.welcomeUser()
        output <- TestConsole.output
      } yield output.map(_.trim)
      assertZIO(logicUnderTest)(Assertion.hasSameElements(List("Please enter your name...", "", "Welcome, Bob")))
    },
    test("ZIO clock") {
      val parallelEffect = for {
        fiber <- ZIO.sleep(5.minutes).timeout(1.minute).fork
        _ <- TestClock.adjust(1.minute) // TestClock - manipulate time
        result <- fiber.join
      } yield result
    ...
    test("ZIO Random") {
      val effect = for {
        _ <- TestRandom.feedInts(3,4,1,2) // TestRandom - control randomness
...
object AspectsSpec extends ZIOSpecDefault {
  ...
  def spec = suite("Testing Aspects")(
    ...
    } @@ timeout(10.millis) @@ timed //Description of how this test should run.
    /**
      Examples of Aspects (see/seen docs):
      - timeout(duration)
